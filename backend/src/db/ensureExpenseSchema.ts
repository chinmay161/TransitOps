...
const statements = [
  ...
  `ALTER TABLE expenses ADD COLUMN IF NOT EXISTS fuel_log_id UUID`,
  ...
  `ALTER TABLE expenses
    ADD CONSTRAINT fk_expenses_fuel_log
    FOREIGN KEY (fuel_log_id) REFERENCES fuel_logs (id) ON DELETE CASCADE`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_expenses_fuel_log_id ON expenses (fuel_log_id)`,
  `ALTER TABLE expenses
